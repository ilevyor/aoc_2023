...
fn parse_list(s: &str) -> Vec<u32> {
    let mut v = s.split_whitespace().map(|n| n.parse::<u32>().unwrap()).collect::<Vec<u32>>();
    v.sort_unstable();
    v
}
...
fn card_copies(cards: &str) -> u32 {
    cards.lines().map(|l| {
        ...
        OrderedTraverse::new(win, nums).filter(|(a, b)| a == b).count()
...
fn lottery_winnings(cards: &str) -> u32 {
    cards.lines().par_bridge().map(|l| {
        let mut  sides = l.split(':').skip(1).next().unwrap().split('|');
        let win = parse_list(sides.next().unwrap());
        let nums = parse_list(sides.next().unwrap());
        OrderedTraverse::new(win, nums).filter(|(a, b)| a == b).fold(1, |e, _| e * 2) / 2
    }).sum()
}