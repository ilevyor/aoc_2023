...
fn update_ratios(ratios: &mut Vec<(usize, usize)>, row: usize, len: usize, (a, b, c): (u8, u8, u8)) {
    if is_digit(b) {
...
fn adj_gear_ratio(top: &mut Vec<u32>, mid: &mut Vec<u32>, bot: &mut Vec<u32>, gear: &mut Vec<((usize, usize), (usize, usize))>, ((a, b, c),(d, e, f),(g, h, i)): ((u8, u8, u8),(u8, u8, u8),(u8, u8, u8))) {
    update_row(top, d);
    update_row(mid, e);
    update_row(bot, f);
    if e == b'*' {
...
fn gear_ratio(schematic: &str) -> u32 {
    ...
    lines.map(|l| [b'.'].into_iter().chain(l.bytes()).chain([b'.', b'.'].into_iter())).tuple_windows::<(_,_,_)>().par_bridge().map(|(a, b, c)| {
        ...
        let mut bot = vec![0];
        let mut gear = vec![];
        izip!(a, b, c).tuple_windows::<(_,_,_)>().for_each(|t| adj_gear_ratio(&mut top, &mut mid, &mut bot, &mut gear, t));
        let nums = [top, mid, bot];
        gear.into_iter().map(|((i1, j1), (i2, j2))| nums[i1][j1] * nums[i2][j2]).sum::<u32>()
    }).sum()
}
